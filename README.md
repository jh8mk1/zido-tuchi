...
## 現在の登録手法（V6.1 ポートフォリオ）

| 手法ID | 種別 | 内容 | 役割 |
|---|---|---|---|
| `daytrade_v5` | デイトレ | 5分ピボN=5 ABC + 15分/1H correct-dow二重一致 + ADX[15,32]レジーム + RR2.0固定 | エンジンA（主力） |
| `swing_s1` | スイング | 4H単一TF EMA30 + RSI45/55押し戻り + ATR×2.0 SL + RR2.5固定・最大7日 | エンジンB（安定化） |

**ポジション管理（手法書 §7-1）**: 2手法が**逆方向**のポジションを同時に持とうとした場合、
後発のエントリーは見送る（`run.py` の `opposite_holder`）。同方向の重複は許可。
V6.1 全期間: 年次PF中央値 1.20 / 最悪年ほぼ収支ゼロ / +11,295pips（往復0.5pips控除・先読み無し）。

> 旧 `daytrade_v4`・`swing_trailing` は `disabled=True` で停止（ファイルは記録として保管）。
> 特に `swing_trailing` の報告値 PF2.30 は MTF ルックアヘッドの産物で、実力は PF≈0.65 の負け手法だった。
